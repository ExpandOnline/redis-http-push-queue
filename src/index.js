...
export default () => {
  ...
  const publishClient = connect(config.get('redis.host'), config.get('redis.port'));
  client.on('message', (channel, message) => {
    const parsedMessage = JSON.parse(message);
    request.post({
      json: true,
      url: parsedMessage.endpoint,
      body: parsedMessage.args,
      ...
      if (any(equals(res.statusCode), config.get('redis.retryCodes'))) {
        setTimeout(() => publishClient.publish(channel, message), 3000);
      }
    });
  });
